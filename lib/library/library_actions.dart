...
class LibraryActions {
  ...
  static LibraryState initializeLibrary(
    LibraryState state,
    InitializeLibraryCommand command,
  ) {
  ...
  static LibraryState libraryInitialized(
    LibraryState state,
    List<Song> library,
    List<Song> favorites,
  ) {
  ...
  static LibraryState libraryInitializationFailed(LibraryState state) {
    return state.copyWith(isLoading: false, library: [], favorites: []);
  }
  ...
  static LibraryState saveSong(LibraryState state, SaveSongCommand command) {
    ...
    return state.copyWith(library: updatedLibrary, statistics: statistics);
  }

  /// Remove a song from the library
  static LibraryState removeSong(
    LibraryState state,
    RemoveSongCommand command,
  ) {
    final updatedLibrary = state.library
        .where((song) => song != command.song)
        .toList();
  ...
  static LibraryState clearLibrary(
    LibraryState state,
    ClearLibraryCommand command,
  ) {
    return state.copyWith(
      library: [],
      favorites: [],
  ...
  static LibraryState toggleFavorite(
    LibraryState state,
    ...
    return state.copyWith(favorites: updatedFavorites, statistics: statistics);
  }

  /// Search songs by query
  static LibraryState searchSongs(
    LibraryState state,
    SearchSongsCommand command,
  ) {
    ...
    return state.copyWith(searchQuery: command.query, searchResults: results);
  }

  /// Clear search and show all songs
  static LibraryState clearSearch(
    LibraryState state,
    ClearSearchCommand command,
  ) {
    return state.copyWith(searchQuery: '', searchResults: []);
  }

  /// Start selection mode for playlist operations
  static LibraryState startSelection(
    LibraryState state,
    StartSelectionCommand command,
  ) {
    return state.copyWith(
  ...
  static LibraryState toggleSelection(
    LibraryState state,
    ToggleSelectionCommand command,
  ) {
    final Set<Song> updatedSelection;
    if (state.selectedSongs.contains(command.song)) {
      // Remove from selection
      updatedSelection = Set<Song>.from(state.selectedSongs)
        ..remove(command.song);
  ...
  static LibraryState selectAll(LibraryState state, SelectAllCommand command) {
    final songsToSelect = state.hasSearchResults
        ? state.searchResults
        : state.library;
    return state.copyWith(selectedSongs: Set<Song>.from(songsToSelect));
  }

  /// Deselect all songs in selection mode
  static LibraryState deselectAll(
    LibraryState state,
    DeselectAllCommand command,
  ) {
    return state.copyWith(isSelectingMode: false, selectedSongs: {});
  }

  /// Finish selection mode and return selected songs
  static LibraryState finishSelection(
    LibraryState state,
    FinishSelectionCommand command,
  ) {
    return state.copyWith(isSelectingMode: false);
  }
  ...
  static LibraryStatistics _calculateStatistics(List<Song> library) {
    return LibraryStatistics(
      ...
      totalFavorites:
          0, // This will be set separately when favorites are loaded
