...
class ThemeActions {
  ...
  static ThemeState initializeThemes(
    ThemeState state,
    InitializeThemesCommand command,
  ) {
    return state.copyWith(isLoading: true, errorMessage: null);
  }

  /// Load all themes and set default
  static ThemeState themesLoaded(
    ThemeState state,
  ...
  static ThemeState themesLoadFailed(ThemeState state, String error) {
    return state.copyWith(isLoading: false, errorMessage: error);
  }
  ...
  static ThemeState switchTheme(ThemeState state, SwitchThemeCommand command) {
    ...
    if (theme != null) {
      return state.copyWith(currentTheme: theme, errorMessage: null);
    }
    return state.copyWith(
  ...
  static Future<Map<String, ThemeConfig>> loadThemeAssets() async {
    try {
      ...
      print(
        'Successfully loaded ${themes.length} themes: ${themes.keys.join(', ')}',
      );
      return themes;
    } catch (e) {
